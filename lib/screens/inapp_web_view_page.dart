...
class _InAppWebPageState extends State<InAppWebPage> {
  ...
  FutureBuilder<bool> futureBuilder() {
    return FutureBuilder<bool>(
        ...
        builder: (context, snapshot) {
          isAndroidTenOrAbove = snapshot.data!;
          return inAppWebView();
        });
  }
  ...
  Future<NavigationActionPolicy> shouldOverrideUrl(controller, request) async {
    // debugPrint("URL Is " + request.request.url.toString());
    var uri = request.request.url;
    if (Platform.isAndroid) {
      bool checkingCondition = uri.toString().startsWith('tel:');
    ...
    if (!["http", "https", "file", "chrome", "data", "javascript", "about"]
        .contains(request.request.url.scheme)) {
  ...
  AndroidInAppWebViewOptions androidInAppWebViewOptions() {
    return AndroidInAppWebViewOptions(
      useHybridComposition: isAndroidTenOrAbove,
